...
if start_idx != -1 and end_idx != -1:
    # Back up a bit to find the actual container end
    end_idx = text.rfind('</div>', start_idx, end_idx) 
    end_idx = text.rfind('</div>', start_idx, end_idx) 
    
    # We can just split by partner-card
    ...
    # The original order: GOG, AMC, ICDS, NABARD, WCD, NARMADA, DRDA, WASMO, GCCI
    new_order = ['GOG', 'AMC', 'ICDS', 'NABARD', 'WCD', 'NARMADA', 'DRDA', 'WASMO', 'GCCI']
